...
const AccountSettings = () => {
  ...
  const handleSubmit = async () => {
    if (!formData.changedImageFile) return toast.info('Logo Not Uploaded');
    if (!formData.country) return toast.info('Country not changed');
    ...
    try {
      ...
      const uploadedLogo = await uploadAttachment(uploadLogoData).unwrap();
  ...
  const handleCancel = () => {
    if (userProfile) {
      setFormData({
        ...
        profileImage: userProfile.vendor_logo?.src || sample,
  ...
  useEffect(() => {
    if (userProfile) {
      setFormData({
        ...
        country: userProfile.country?.id || null,
        other_links: userProfile.other_links || ["", "", ""],
        profileImage: userProfile.vendor_logo?.url || sample,
        user_name: userProfile.vendor_name || '',
        social_media: userProfile.social_media || ["", "", ""],
        website_url: userProfile.website_url || ''
      });
